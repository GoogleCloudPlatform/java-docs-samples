...
public class CreateSecurityHealthAnalyticsCustomModule {
	...
	public static SecurityHealthAnalyticsCustomModule createSecurityHealthAnalyticsCustomModule(String parent,
			String customModuleDisplayName) throws IOException {
		...
		try (SecurityCenterManagementClient client = SecurityCenterManagementClient.create()) {
			...
			// define the custom module configuration, update the severity, description, recommendation below
			...
			// define the security health analytics custom module configuration, update the EnablementState below
			SecurityHealthAnalyticsCustomModule securityHealthAnalyticsCustomModule = SecurityHealthAnalyticsCustomModule
					.newBuilder().setName(name).setDisplayName(customModuleDisplayName)
					.setEnablementState(EnablementState.ENABLED).setCustomConfig(customConfig).build();
			...
			CreateSecurityHealthAnalyticsCustomModuleRequest request = CreateSecurityHealthAnalyticsCustomModuleRequest
					...
					.setSecurityHealthAnalyticsCustomModule(securityHealthAnalyticsCustomModule).build();

			// calls the API
			SecurityHealthAnalyticsCustomModule response = client.createSecurityHealthAnalyticsCustomModule(request);

			System.out.printf("SecurityHealthAnalyticsCustomModule created : %s", response);
