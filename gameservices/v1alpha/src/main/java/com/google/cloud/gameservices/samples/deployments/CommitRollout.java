...
public class CommitRollout {
  public static void commitRollout(String deploymentName) {
    ...
    try (GameServerDeploymentsServiceClient client = GameServerDeploymentsServiceClient.create()) {
      OperationFuture<GameServerDeployment, Empty> call = client.commitRolloutAsync(deploymentName);

      GameServerDeployment result = call.get(1, TimeUnit.MINUTES);
      System.out.println("Rollout committed: " + result.getName());
    } catch (IOException | InterruptedException | ExecutionException | TimeoutException e) {
      System.err.println("Commit Rollout request unsuccessful.");
