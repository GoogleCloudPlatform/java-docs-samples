...
public class ClustersTest {
    ...
    protected static final String clusterName = "projects/test-project/locations/us-central1/clusters/test-cluster";
    ...
    public void createClusterTest() throws Exception {
        ...
        try (MockedStatic<ManagedKafkaClient> mockedStatic = Mockito.mockStatic(ManagedKafkaClient.class)) {
            ...
            when(managedKafkaClient.createClusterOperationCallable()).thenReturn(MockOperationFuture.getOperableCallable());
    ...
    public void listClustersTest() throws Exception {
        ManagedKafkaClient managedKafkaClient = mock(ManagedKafkaClient.class);
        ManagedKafkaClient.ListClustersPagedResponse response = mock(ManagedKafkaClient.ListClustersPagedResponse.class);
        try (MockedStatic<ManagedKafkaClient> mockedStatic = Mockito.mockStatic(ManagedKafkaClient.class)) {
            mockedStatic.when(() -> create()).thenReturn(managedKafkaClient);
            Iterable<Cluster> iterable = () -> {
                Cluster cluster = Cluster.newBuilder()
                        .setName(ClusterName.of(projectId, region, clusterId).toString())
                        .build();
                List<Cluster> list = new ArrayList<Cluster>(Arrays.asList(cluster));
    ...
    public void updateClusterTest() throws Exception {
        ...
        try (MockedStatic<ManagedKafkaClient> mockedStatic = Mockito.mockStatic(ManagedKafkaClient.class)) {
            mockedStatic.when(() -> create()).thenReturn(managedKafkaClient);
            when(managedKafkaClient.updateClusterOperationCallable()).thenReturn(MockOperationFuture.getOperableCallable());
    ...
    public void deleteClusterTest() throws Exception {
        ...
        try (MockedStatic<ManagedKafkaClient> mockedStatic = Mockito.mockStatic(ManagedKafkaClient.class)) {
            mockedStatic.when(() -> create()).thenReturn(managedKafkaClient);
            when(managedKafkaClient.deleteClusterOperationCallable()).thenReturn(MockDeleteOperationFuture.getOperableCallable());
