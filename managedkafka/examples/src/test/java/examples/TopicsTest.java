...
public class TopicsTest {
    ...
    public void createTopicTest() throws Exception {
        ...
        try (MockedStatic<ManagedKafkaClient> mockedStatic = Mockito.mockStatic(ManagedKafkaClient.class)) {
            ...
            Topic topic = Topic.newBuilder()
                    .setName(TopicName.of(ClustersTest.projectId, ClustersTest.region, ClustersTest.clusterId, topicId).toString())
                    .build();
            ...
            CreateTopic.createTopic(ClustersTest.projectId, ClustersTest.region, ClustersTest.clusterId,
                    topicId, partitionCount, replicationFactor, configs);
    ...
    public void getTopicTest() throws Exception {
        ...
        try (MockedStatic<ManagedKafkaClient> mockedStatic = Mockito.mockStatic(ManagedKafkaClient.class)) {
            ...
            GetTopic.getTopic(ClustersTest.projectId, ClustersTest.region, ClustersTest.clusterId, topicId);
            String output = bout.toString();
            assertThat(output).contains(topicName);
    ...
    public void listTopicsTest() throws Exception {
        ManagedKafkaClient managedKafkaClient = mock(ManagedKafkaClient.class);
        ManagedKafkaClient.ListTopicsPagedResponse response = mock(ManagedKafkaClient.ListTopicsPagedResponse.class);
        try (MockedStatic<ManagedKafkaClient> mockedStatic = Mockito.mockStatic(ManagedKafkaClient.class)) {
            mockedStatic.when(() -> create()).thenReturn(managedKafkaClient);
            Iterable<Topic> iterable = () -> {
                Topic topic = Topic.newBuilder()
                        .setName(TopicName.of(ClustersTest.projectId, ClustersTest.region,
                                ClustersTest.clusterId, topicId).toString())
                        .build();
                List<Topic> list = new ArrayList<Topic>(Arrays.asList(topic));
    ...
    public void updateTopicTest() throws Exception {
        ...
        try (MockedStatic<ManagedKafkaClient> mockedStatic = Mockito.mockStatic(ManagedKafkaClient.class)) {
            ...
            UpdateTopic.updateTopic(ClustersTest.projectId, ClustersTest.region, ClustersTest.clusterId,
                    topicId, partitionCount, configs);
            String output = bout.toString();
    ...
    public void deleteTopicTest() throws Exception {
        ...
        try (MockedStatic<ManagedKafkaClient> mockedStatic = Mockito.mockStatic(ManagedKafkaClient.class)) {
            mockedStatic.when(() -> create()).thenReturn(managedKafkaClient);
            DeleteTopic.deleteTopic(ClustersTest.projectId, ClustersTest.region, ClustersTest.clusterId, topicId);
            String output = bout.toString();
