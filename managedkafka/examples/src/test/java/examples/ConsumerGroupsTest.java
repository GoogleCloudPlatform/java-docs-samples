...
import com.google.cloud.managedkafka.v1.*;
...
public class ConsumerGroupsTest {
    ...
    protected static final String consumerGroupName =
            "projects/test-project/locations/us-central1/clusters/test-cluster/consumerGroups/test-consumer-group";
    ...
    public void getConsumerGroupTest() throws Exception {
        ...
        try (MockedStatic<ManagedKafkaClient> mockedStatic = Mockito.mockStatic(ManagedKafkaClient.class)) {
            mockedStatic.when(() -> create()).thenReturn(managedKafkaClient);
            ConsumerGroup consumerGroup = ConsumerGroup.newBuilder()
                    .setName(ConsumerGroupName.of(ClustersTest.projectId, ClustersTest.region,
                            ClustersTest.clusterId, consumerGroupId).toString())
                    .build();
            when(managedKafkaClient.getConsumerGroup(any(ConsumerGroupName.class))).thenReturn(consumerGroup);
            GetConsumerGroup.getConsumerGroup(ClustersTest.projectId, ClustersTest.region,
                    ClustersTest.clusterId, consumerGroupId);
    ...
    public void listConsumerGroupsTest() throws Exception {
        ...
        try (MockedStatic<ManagedKafkaClient> mockedStatic = Mockito.mockStatic(ManagedKafkaClient.class)) {
            ...
            Iterable<ConsumerGroup> iterable = () -> {
                ConsumerGroup consumerGroup = ConsumerGroup.newBuilder()
                        .setName(ConsumerGroupName.of(ClustersTest.projectId, ClustersTest.region,
                                ClustersTest.clusterId, consumerGroupId).toString())
                        .build();
                List<ConsumerGroup> list = new ArrayList<ConsumerGroup>(Arrays.asList(consumerGroup));
            ...
            ListConsumerGroups.listConsumerGroups(ClustersTest.projectId, ClustersTest.region,
    ...
    public void updateConsumerGroupTest() throws Exception {
        ...
        try (MockedStatic<ManagedKafkaClient> mockedStatic = Mockito.mockStatic(ManagedKafkaClient.class)) {
            ...
            Map<Integer, Integer> partitionOffsets = new HashMap<Integer, Integer>() {{
                put(1,10);
                put(2,20);
                put(3,30);
            }};
            UpdateConsumerGroup.updateConsumerGroup(ClustersTest.projectId, ClustersTest.region,
                    ClustersTest.clusterId, TopicsTest.topicId, consumerGroupId, partitionOffsets);
            String output = bout.toString();
            assertThat(output).contains("Updated consumer group");
            assertThat(output).contains(consumerGroupName);
            verify(managedKafkaClient, times(1)).updateConsumerGroup(
                    any(UpdateConsumerGroupRequest.class));
        }
    }
    ...
    public void deleteConsumerGroupTest() throws Exception {
        ...
        try (MockedStatic<ManagedKafkaClient> mockedStatic = Mockito.mockStatic(ManagedKafkaClient.class)) {
            mockedStatic.when(() -> create()).thenReturn(managedKafkaClient);
            DeleteConsumerGroup.deleteConsumerGroup(ClustersTest.projectId, ClustersTest.region,
                    ClustersTest.clusterId, consumerGroupId);
            String output = bout.toString();
