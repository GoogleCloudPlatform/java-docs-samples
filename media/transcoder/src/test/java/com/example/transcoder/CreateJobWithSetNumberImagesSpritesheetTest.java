...
public class CreateJobWithSetNumberImagesSpritesheetTest {
  ...
  private static final String OUTPUT_DIR = "test-output-set-number-spritesheet/";
  private static final String OUTPUT_URI_FOR_SET_NUMBER_SPRITESHEET =
      "gs://" + BUCKET_NAME + "/" + OUTPUT_DIR;
  ...
  private static boolean checkFileExists(String bucketName, String fileName) {
    Storage storage = StorageOptions.newBuilder().setProjectId(PROJECT_ID).build().getService();
  ...
  @Test
  public void test_CreateJobWithSetNumberImagesSpritesheet() throws Exception {
    String jobName = String.format("projects/%s/locations/%s/jobs/", PROJECT_NUMBER, LOCATION);
    CreateJobWithSetNumberImagesSpritesheet.createJobWithSetNumberImagesSpritesheet(
    ...
    assertTrue(
        "Check that small spritesheet exists.",
        checkFileExists(
            BUCKET_NAME,
            OUTPUT_DIR
                + CreateJobWithSetNumberImagesSpritesheet.smallSpritesheetFilePrefix
                + CreateJobWithSetNumberImagesSpritesheet.spritesheetFileSuffix));
    assertTrue(
        "Check that large spritesheet exists.",
        checkFileExists(
            BUCKET_NAME,
            OUTPUT_DIR
                + CreateJobWithSetNumberImagesSpritesheet.largeSpritesheetFilePrefix
                + CreateJobWithSetNumberImagesSpritesheet.spritesheetFileSuffix));
  }
