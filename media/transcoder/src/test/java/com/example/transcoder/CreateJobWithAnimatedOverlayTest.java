...
public class CreateJobWithAnimatedOverlayTest {
  ...
  private static final String TEST_OVERLAY_NAME = "overlay.jpg";
  private static final String TEST_OVERLAY_PATH =
      "src/test/java/com/example/transcoder/testdata/" + TEST_OVERLAY_NAME;
  private static final String OVERLAY_URI = "gs://" + BUCKET_NAME + "/" + TEST_OVERLAY_NAME;
  ...
  public void beforeTest() throws IOException {
    ...
    blobId = BlobId.of(BUCKET_NAME, TEST_OVERLAY_NAME);
    blobInfo = BlobInfo.newBuilder(blobId).build();
    path = Paths.get(TEST_OVERLAY_PATH);
    storage.create(blobInfo, Files.readAllBytes(path));
  ...
  @Test
  public void test_CreateJobWithAnimatedOverlay() throws Exception {
    String jobName = String.format("projects/%s/locations/%s/jobs/", PROJECT_NUMBER, LOCATION);
    CreateJobWithAnimatedOverlay.createJobWithAnimatedOverlay(
        PROJECT_ID, LOCATION, INPUT_URI, OVERLAY_URI, OUTPUT_URI_FOR_AD_HOC);
