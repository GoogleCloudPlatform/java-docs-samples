...
public class CreateJobWithConcatenatedInputsTest {
  ...
  private static final String TEST_FILE_1_NAME = "ForBiggerEscapes.mp4";
  private static final String TEST_FILE_1_PATH =
      "src/test/java/com/example/transcoder/testdata/" + TEST_FILE_1_NAME;
  private static final String INPUT_1_URI = "gs://" + BUCKET_NAME + "/" + TEST_FILE_1_NAME;
  private static final String TEST_FILE_2_NAME = "ForBiggerJoyrides.mp4";
  private static final String TEST_FILE_2_PATH =
      "src/test/java/com/example/transcoder/testdata/" + TEST_FILE_2_NAME;
  private static final String INPUT_2_URI = "gs://" + BUCKET_NAME + "/" + TEST_FILE_2_NAME;
  private static final String OUTPUT_URI_FOR_CONCAT =
      "gs://" + BUCKET_NAME + "/test-output-concat/";
  ...
  public void beforeTest() throws IOException {
    ...
    BlobId blobId = BlobId.of(BUCKET_NAME, TEST_FILE_1_NAME);
    BlobInfo blobInfo = BlobInfo.newBuilder(blobId).build();
    Path path = Paths.get(TEST_FILE_1_PATH);
    storage.create(blobInfo, Files.readAllBytes(path));

    blobId = BlobId.of(BUCKET_NAME, TEST_FILE_2_NAME);
    blobInfo = BlobInfo.newBuilder(blobId).build();
    path = Paths.get(TEST_FILE_2_PATH);
    storage.create(blobInfo, Files.readAllBytes(path));
