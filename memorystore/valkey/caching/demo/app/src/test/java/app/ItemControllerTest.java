...
class ItemControllerTest {
  ...
  void testReadItem() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            content()
                .string(
                    Matchers.containsString("\"id\":1"))) // Assert JSON contains "id":1
        .andExpect(
            content()
                .string(
                    Matchers
                        .containsString("\"name\":\"ItemName\""))) // Assert JSON contains name
        .andExpect(
            content()
                .string(
                    Matchers
  ...
  void testReadMultipleItems() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            status()
                .isOk()) // Assert HTTP status is 200 OK
        .andExpect(
            jsonPath("$.items.length()")
                .value(10)) // Assert the `items` array has 10 elements
        .andExpect(
            jsonPath("$.items[0].id")
                .value(1)) // Check first item's ID
        .andExpect(
            jsonPath("$.items[0].name")
                .value("Item1")) // Check first item's name
        .andExpect(
            jsonPath("$.items[9].id")
                .value(10)) // Check last item's ID
        .andExpect(
            jsonPath("$.items[9].name")
                .value("Item10")); // Check last item's name

    // Assert: Verify DataController's getMultiple method was called with the
    // correct parameter
  ...
  void testCreateItem() throws Exception {
    ...
    // Assert: Verify DataController's create method was called with an argument
  ...
  void testCreateItem_Validations() throws Exception {
    // Act: Perform POST /item/create with an invalid request body (missing required
    // fields)
    mockMvc
        .perform(
            post("/api/item/create")
