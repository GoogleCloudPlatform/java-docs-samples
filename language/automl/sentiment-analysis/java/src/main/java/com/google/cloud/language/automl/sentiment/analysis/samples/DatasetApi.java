...
/**
 * Google Cloud AutoML Natural Language Sentiment Analysis API sample application. Example usage:
 * mvn package exec:java
 * -Dexec.mainClass='com.google.cloud.language.automl.sentiment.analysis.samples.DatasetApi'
...
public class DatasetApi {
   ...
   * @param sentimentMax the sentiment value is an integer ranging from 0 (relatively negative) to a
   *     maximum value of 10 (positive).
   * @throws IOException
   */
  ...
  // [START automl_natural_language_sentiment_list_datasets]
  /**
  ...
  public void argsHelper(String[] args)
    ...
    createDatasetParser.addArgument("sentimentMax");

    Subparser listDatasetsParser = subparsers.addParser("list_datasets");
    listDatasetsParser
        .addArgument("filter")
        .nargs("?")
        .setDefault("textSentimentDatasetMetadata:*");
    ...
    try {
      ...
      if (ns.get("command").equals("create_dataset")) {
        createDataset(
            projectId, computeRegion, ns.getString("datasetName"), ns.getString("sentimentMax"));
      }
